...
class DataLoaderModel(DataLoaderBase):
    ...
    def construct_data(self, kg_data):
        ...
        self.n_patients_entities = max(max(kg_data['h']),max(kg_data['t'])) + 1
        self.n_entities = self.n_patients_entities - self.n_patients
        ...
        self.train_patient_dict = {k: np.unique(v).astype(np.int32) for k, v in self.train_patient_dict.items()}
        self.test_patient_dict = {k: np.unique(v).astype(np.int32) for k, v in self.test_patient_dict.items()}
    ...
    def create_adjacency_dict(self):
        ...
        for r, ht_list in self.train_relation_dict.items():
            ...
            vals = [1] * len(rows)
            adj = sp.coo_matrix((vals, (rows, cols)), shape=(self.n_patients_entities, self.n_patients_entities))
    ...
    def print_info(self, logging):
        ...
        logging.info('n_patients_entities:  %d' % self.n_patients_entities)
