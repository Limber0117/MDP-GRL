...
def evaluate(model, dataloader, Ks, device):
    """
       ...
       Args:
           ...
           dataloader: DataLoader containing test data and metadata.
           Ks: List of top-K values for evaluation metrics.
           device: Computational device (CPU or GPU).

       Returns:
           cf_scores: Predicted scores for all patient-disease pairs.
           metrics_dict: Dictionary containing evaluation metrics (precision, recall, F1, NDCG) at various K values.
       """
    ...
    with tqdm(total=len(patient_ids_batches), desc='Evaluating Iteration') as pbar:
        ...
        logging.info('Total Testing Time: {:.1f}s'.format(total_test_time))
...
def train(args):
    """
        Train the MDP-GRL model using Patient-Disease Bipartite Graph and Local Knowledge Graph information.
    """
    # seed
    ...
    if args.patient_pretrain == 1:
        patient_pre_embed = torch.tensor(data.user_pre_embed)
        disease_pre_embed = torch.tensor(data.item_pre_embed)
    else:
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        if epoch % args.evaluate_every == 0 or epoch == args.n_epoch:
            ...
            ## Save CF scores to file
            # scores_file = f"{args.save_dir}/cf_scores_epoch_{epoch}.npy"
            # np.save(scores_file, cf_scores)
            # logging.info(f'Saved CF scores to {scores_file}')
            ...
            for k, metrics in metrics_dict.items():
                ...
                print(f"Precision: {precision}, Recall: {recall}, F1: {f1}, NDCG: {ndcg}")
            ...
            if current_ndcg > best_ndcg_score:
                # 调用 save_model，并接收返回的 best_ndcg（即 current_ndcg）
                best_ndcg_score = save_model(model, args.save_dir, epoch, current_ndcg, last_best_epoch=best_epoch,
                                             last_best_ndcg=best_ndcg_score)
                logging.info(f'Save best NDCG@{k_min} model on epoch {epoch}!')
                best_epoch = epoch
